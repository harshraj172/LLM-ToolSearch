...
class QueryEvalCallback(TrainerCallback):
    def __init__(self, test_dataset, logger, args: TrainingArguments, tokenizer: T5Tokenizer):
        ...
        for batch in tqdm(self.dataloader, desc='Evaluating dev queries'):
            ...
            with torch.no_grad():
                ...
                for beams, label in zip(batch_beams, labels):
                    ...
                    print('np.array(rank_list)[:10] = ', np.array(rank_list)[:10])
...
def main():
    ...
    wandb.login()
    wandb.init(project="llm_tool_search", name='chatweb3-finetune-t5-base')
    ...
    train_dataset = IndexingTrainDataset(path_to_data='data-finetune/chatweb3/chatweb3_train.json',
                                         max_length=L,
                                         cache_dir='cache',
                                         tokenizer=tokenizer,
                                         finetune=True)
    ...
    eval_dataset = IndexingTrainDataset(path_to_data='data-finetune/chatweb3/chatweb3_train.json',
                                        max_length=L,
                                        cache_dir='cache',
                                        tokenizer=tokenizer,
                                        finetune=True)
    ...
    test_dataset = IndexingTrainDataset(path_to_data='data-finetune/chatweb3/chatweb3_valid.json',
                                        max_length=L,
                                        cache_dir='cache',
                                        tokenizer=tokenizer,
                                        finetune=True)
    ...
    trainer = IndexingTrainer(
        ...
        callbacks=[QueryEvalCallback(test_dataset, wandb, training_args, tokenizer)],
    )
    trainer.train(
    )
